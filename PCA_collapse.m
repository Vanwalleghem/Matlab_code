...
listOfPCAmat = dir('PCA-*.mat');
numberOfPCA = numel(listOfPCAmat);
mydata(numberOfPCA).comps=0;
mydata(numberOfPCA).W=0;
for j = 1:numberOfPCA
    try
    myfilename = sprintf('PCA-%d.mat',j);
    mydata(j)=load(myfilename);
    end
end


for j = 1:NbRepeats:numberOfPCA
    F=[];
    G=[];
    %try
   for k=0:(NbRepeats-1)
    F=horzcat(F,mydata(j+k).comps);
    ...
    start=0;
    temp=0;
    cr2=reshape(cr,NbRepeats*size(mydata(1).comps,2),NbRepeats*size(mydata(1).comps,2));
    allcr{j}=cr2;    
    ...
    for idx = 1:numel(test2)
        element = test2(idx);
        x = ceil(element/(NbRepeats*size(mydata(1).comps,2)));
        y = mod(element,NbRepeats*size(mydata(1).comps,2));
        if y == 0
            y=NbRepeats*size(mydata(1).comps,2);
        end
        if any(temp == y)
        else
            temp(end+1) = y;
...
save('PCA - collapse.mat','Hval','Wval','-v7.3')
save('cross-correlation - PCA.mat','allcr','-v7.3')
close all;
...
for i=1:size(Hval,2)
	if isempty(Hval{i})
		...
		for k=1:size(Hval{i},2)
			...
			myfilename = sprintf('PCAcollapse-%d-%d.png',i,k);
			saveas(f, myfilename,'png');
            close all;
		end
	end
...
%aud8freq
    aud8freq=zeros(8,size(Hval{1,1},1));
...
for i=1:size(Hval,2)
	if isempty(Hval{i})
		...
		myfilename = sprintf('PCAcorrelation-%d.png',i);
		saveas(f, myfilename,'png');
        close all;
		test2=find(ccf>0.5);
		for idx = 1:numel(test2)
			...
			myfilename = sprintf('PCA-%d_%d_stim-%d.png',i,y,x);
			saveas(f, myfilename,'png');
            close all;
		end
	end
